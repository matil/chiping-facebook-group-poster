...
| `CLOUDFLARE_API_TOKEN` | Deploys the isolated remote-login Pages shell |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare account containing the Pages shell |
...
or publish a post. A dedicated Chiping-owned Pages site serves noVNC at
`https://fb-login.chiping.co.il`; its encrypted WebSocket backend is offline
outside that workflow. Only enable the posting secret after that check succeeds.
